...
/**
 * Deduplicate Rwanda Contacts Script
 * 
 * This script will deduplicate the Rwanda author profiles against existing contacts
...
// Load the authors
const authors = JSON.parse(fs.readFileSync(path.join(__dirname, 'rwanda-authors.json'), 'utf-8'));
...
function saveResearchLog() {
  fs.writeFileSync(
    path.join(__dirname, 'rwanda-deduplication-log.json'),
    JSON.stringify(researchLog, null, 2)
  );
  console.log('Deduplication log saved to rwanda-deduplication-log.json');
}
...
async function deduplicateContacts() {
  ...
  // Save unique authors
  fs.writeFileSync(
    path.join(__dirname, 'rwanda-authors-unique.json'),
