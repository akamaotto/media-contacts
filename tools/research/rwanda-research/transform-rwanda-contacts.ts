...
/**
 * Transform Rwanda Contacts Script
 * 
...
const enrichedAuthors = JSON.parse(fs.readFileSync(path.join(__dirname, 'rwanda-authors-enriched.json'), 'utf-8'));

// Load outlet data
const outletsData = JSON.parse(fs.readFileSync(path.join(__dirname, 'rwanda-outlets.json'), 'utf-8'));
...
function saveResearchLog() {
  fs.writeFileSync(
    path.join(__dirname, 'rwanda-transform-log.json'),
    JSON.stringify(researchLog, null, 2)
  );
  console.log('Transform log saved to rwanda-transform-log.json');
}

// Function to get Rwanda country ID
async function getRwandaCountryId(): Promise<string> {
  const country = await prisma.countries.findFirst({
    where: { name: { equals: 'Rwanda', mode: 'insensitive' } },
    select: { id: true }
  });
  return country?.id || '4e75e89d-81ca-46a7-9ec7-85edb192f8e8'; // Default to the ID we found earlier
}
...
async function transformAuthor(author: any): Promise<any> {
  ...
  const countryId = await getRwandaCountryId();
...
async function transformContacts() {
  ...
  fs.writeFileSync(
    path.join(__dirname, 'rwanda-authors-transformed.json'),
    JSON.stringify(transformedAuthors, null, 2)
  );
  
  fs.writeFileSync(
    path.join(__dirname, 'rwanda-outlets-transformed.json'),
    JSON.stringify(transformedOutlets, null, 2)
  );
  
  fs.writeFileSync(
    path.join(__dirname, 'rwanda-publishers-transformed.json'),
