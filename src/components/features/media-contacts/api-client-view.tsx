...
import { ClientErrorHandler, isErrorResponse, type APIResponse } from '@/lib/api/client-error-handler';
import { cachedFetch, defaultCache, createCacheKey, withProgressiveEnhancement } from '@/lib/caching/client-cache';
import { ProgressiveEnhancement } from '@/components/ui/error-boundary';
import { useMediaContactsShortcuts, ScreenReaderAnnouncer } from '@/components/ui/accessibility';
...
export default function ApiMediaContactsClientView({
    ...
    const quietReferenceErrorHandler = useMemo(() => new ClientErrorHandler({
        enableToasts: false,
        enableConsoleLogging: false,
    }), []);
    ...
    const [emailVerified, setEmailVerified] = useState<'all' | 'verified' | 'unverified'>('all');
    ...
    const handleBulkDelete = useCallback(async (ids: string[]) => {
        try {
            ...
            ScreenReaderAnnouncer.announce(`Deleted ${ids.length} contacts`);
        } catch (error) {
        ...
        try {
            const response = await errorHandler.fetchWithRetry(
                '/api/media-contacts/export',
                {
                    ...
                    body: JSON.stringify({
                        fields: ['name', 'email', 'title', 'outlets', 'beats', 'countries'],
                        filters: { ids }
                    })
                },
                { maxRetries: 1 }
    ...
    const loadReferenceData = useCallback(async (_isRetry = false) => {
        setIsLoadingReference(true);

        try {
            console.log('🚀 [API-CLIENT] Loading reference data with cache and fallback...');
                ...
                async () => {
                    return await cachedFetch(
                        ...
                        async () => {
                            ...
                            const abortTimer = setTimeout(() => controller.abort(), perRequestTimeout);
                            try {
                                const response = await quietReferenceErrorHandler.fetchWithRetry<ReferenceData>(
                                    '/api/reference',
                                    { method: 'GET', signal: controller.signal, cache: 'no-store' },
                                    {
                                        maxRetries: 1,
                                        baseDelayMs: 500,
                                        retryableCategories: ['network', 'database_connection', 'server', 'timeout', 'rate_limit']
                                    }
                                );

                                const result = quietReferenceErrorHandler.handleResponse(response, {
            ...
            console.error('❌ [API-CLIENT] All reference data loading methods failed:', error);
    ...
    const handleViewContact = useCallback(async (contact: FastTableContact) => {
        ...
        try {
            ...
            const result = errorHandler.handleResponse(response, {
                ...
                customErrorHandler: (error) => {
                    const msg = (error as any)?.error?.message || (error as any)?.message || 'Request failed';
                    console.warn('[VIEW] Contact details failed:', msg, error);
                }
            });

            if (result.success && result.data?.contact) {
                // Update with fresh contact details from database
                setViewingContact(adaptApiContactToTableItem(result.data.contact));
            }
    ...
    const handleEditContact = useCallback(async (contact: FastTableContact) => {
        try {
            ...
            const result = errorHandler.handleResponse(response, {
                ...
                customErrorHandler: (error) => {
                    const msg = (error as any)?.error?.message || (error as any)?.message || 'Request failed';
                    console.warn('[EDIT] Contact details failed:', msg, error);
                }
            });

            if (result.success && result.data?.contact) {
                const adaptedContact = adaptApiContactToTableItem(result.data.contact);
            ...
            // Only catch actual network/unexpected errors, not API errors
            console.error('Unexpected error loading contact for editing:', error);
            toast.error('Unexpected error', {
                description: 'Please try again.',
                duration: 5000
            });
    ...
    const handleDeleteContact = useCallback(async (contact: FastTableContact) => {
    ...
    const handleEnrichmentComplete = useCallback(async (contactId: string) => {
        try {
        ...
        return (
            <div className='space-y-6'>
                <div className="flex items-center justify-center py-12">
                    <div className="text-center space-y-2">
                        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto" />
                        <p className="text-sm text-muted-foreground">Preparing data...</p>
                    </div>
                </div>
            </div>
        );
        ...
        <div className='space-y-6'>
            ...
            <ViewMediaContactSheet
                ...
                onContactDelete={(contactId) => {
                    ...
                    const contactToDelete = allContacts.find(contact => contact.id === contactId);
                    if (contactToDelete) {
                        handleDeleteContact(contactToDelete);
                    } else {
                        // Fallback: if contact not found in allContacts, create a minimal contact object
                        handleDeleteContact({ id: contactId } as FastTableContact);
                    }
                }}
                onContactEdit={(contact) => {
                    ...
                    const fastTableContact: FastTableContact = {
                        ...
                        email: contact.email,
                        title: contact.title,
                        email_verified_status: contact.emailVerified ?? contact.email_verified_status ?? false,
                        updated_at: contact.updatedAt || (typeof contact.updated_at === 'string' ? contact.updated_at : contact.updated_at?.toISOString()) || new Date().toISOString(),
                        outlets: contact.outlets || [],
                        beats: contact.beats || [],
                        ai_beats: contact.ai_beats,
                        countries: contact.countries || [],
                        regions: contact.regions || [],
                        languages: (contact.languages || []).filter(lang => lang.id).map(lang => ({
                            id: lang.id!,
                            name: lang.name,
                            code: lang.code
                        })),
            ...
            <ApiMediaContactsFilters
                ...
                countries={(referenceData?.countries || []).map(c => ({ ...c, label: c.name }))}
                beats={(referenceData?.beats || []).map(b => ({ ...b, label: b.name }))}
                outlets={(referenceData?.outlets || []).map(o => ({ ...o, label: o.name }))}
                regions={(referenceData?.regions || []).map(r => ({ ...r, label: r.name }))}
                languages={(referenceData?.languages || []).map(l => ({ ...l, label: l.name }))}
            />
