...
test.describe('Media Contacts Egypt Filter Test', () => {
  test('should find Egypt when searching countries filter', async ({ page }) => {
    // Use reliable authentication with seeded test users
    ...
    // Wait for filters to be ready (with shorter timeout for faster feedback)
  ...
  test('should handle filter UI components properly', async ({ page }) => {
    // Quick smoke test for filter UI components
    await page.goto('http://localhost:3000/login');
    await page.fill('input[type="email"]', 'test@test.com');
    await page.fill('input[type="password"]', 'test@123');
    await page.click('button[type="submit"]');
    ...
    // Test all filter components are present and functional
    ...
    for (const selector of filterSelectors) {
      ...
      await expect(page.locator('[role="combobox"]')).toBeVisible({ timeout: 2000 });

      // Close dropdown
      await page.keyboard.press('Escape');

      console.log(`✅ ${selector} filter functional`);
    }

    console.log('✅ All filter components working correctly');
  });
});
