...
// Senegal Business Seeding Script
// Goal: Seed 20–35 unique Senegal Business-relevant media contacts with strict deduplication
...
// Publisher mapping for Senegal outlets
const PUBLISHER_MAP: Record<string, { name: string; website?: string; description?: string }> = {
  'Senego': { name: 'Senego Publisher', website: 'https://www.senego.com', description: 'Publisher of Senego news portal.' },
  'Dakaractu': { name: 'Dakaractu Publisher', website: 'https://www.dakaractu.com', description: 'Publisher of Dakaractu news website.' },
  'APS Actualité': { name: 'APS Publisher', website: 'https://www.aps.sn', description: 'Publisher of Agence de Presse Sénégalaise.' },
  'Walfadjri': { name: 'Walfadjri Publisher', website: 'https://www.walfadjri.com', description: 'Publisher of Walfadjri online newspaper.' },
  'Sud Quotidien': { name: 'Sud Quotidien Publisher', website: 'https://www.sudonline.sn', description: 'Publisher of Sud Quotidien newspaper.' },
};
...
const transformedAuthorsPath = path.join(__dirname, '../../senegal-authors-transformed.json');
const transformedOutletsPath = path.join(__dirname, '../../senegal-outlets-real.json');
...
async function seed() {
  ...
  console.log(`[Senegal Business Seed] Starting. DRY_RUN=${DRY_RUN}`);

  const senegalId = await ensureCountryIdByName(COUNTRY_NAME);
  if (!senegalId) throw new Error(`Country not found: ${COUNTRY_NAME}. Please seed countries first.`);
  ...
  for (const contact of transformedAuthors) {
    try {
      ...
      const outletId = await ensureOutlet(outletName, { 
        ...
        description: outletData?.description || `Business outlet in Senegal`
      });
      ...
      const created = await prisma.media_contacts.create({
        data: {
          ...
          updated_at: new Date(),
          // relations
          countries: { connect: [{ id: senegalId }] },
  ...
  console.log(`[Senegal Business Seed] Done in ${elapsed}s. Created=${results.created}, Skipped=${results.skipped}, Errors=${results.errors.length}`);
  ...
  try {
    ...
    const outPath = path.join(outDir, `senegal-business-seed-${DRY_RUN ? 'dry' : 'commit'}-${ts}.json`);
    ...
    console.log(`[Senegal Business Seed] Wrote report: ${outPath}`);
  } catch (e) {
    console.warn('[Senegal Business Seed] Failed to write report file:', e);
  }
}

seed()
  .catch((e) => {
    console.error('[Senegal Business Seed] Unhandled error:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
