 ...
 */
export function startOperation(id: string, type: 'import' | 'export', fileSize?: number) {
 ...
 */
export function completeOperation(id: string, result: { 
  success: boolean;
  rows?: number;
  errors?: any[];
}) {
 ...
 */
export function logError(id: string, error: Error, context?: Record<string, any>) {
  const operation = activeOperations.get(id);
...
export function resetMetrics() {
  Object.keys(metrics.imports).forEach(key => {
    (metrics.imports as any)[key] = 0;
  });
  
  Object.keys(metrics.exports).forEach(key => {
    (metrics.exports as any)[key] = 0;
  });
}
